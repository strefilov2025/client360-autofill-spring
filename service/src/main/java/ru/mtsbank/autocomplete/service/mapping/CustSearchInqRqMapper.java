...
public interface CustSearchInqRqMapper {
    ...
    @Mapping(target = "bankSvcRq.custInfo.personInfo.contactInfo.phoneNum", expression = "java(toPhone(customerFuzzySearchRequest))")
    ...
    @Mapping(target = "bankSvcRq.method", source = "requestCust.method")
    CustSearchListInqRq toFdx(RequestCust requestCust);
    ...
    default PhoneNum toPhone(final CustomerFuzzySearchRequest customerFuzzySearchRequest) {

        if (customerFuzzySearchRequest.getPhones() != null && !customerFuzzySearchRequest.getPhones().isEmpty()) {
            return toPhone(customerFuzzySearchRequest.getPhones().get(0));
        }
        return null;
    }
